...
// syncBackendServices creates and delete backend according to groupBackendServices result.
func (r *ReconcilerBalancer) syncBackendServices(balancer *exposerv1alpha1.Balancer) error {
	...
	for i := range backendServicesToDelete {
		svcDelete := backendServicesToDelete[i]
...
func groupBackendServers(balancer *exposerv1alpha1.Balancer, currentBackendServices []corev1.Service) (backendServicesToCreate []corev1.Service, backendServicesToDelete []corev1.Service, activeBackendServices []corev1.Service) {
	...
	for _, port := range balancer.Spec.Ports {
		balancerPorts = append(balancerPorts, corev1.ServicePort{
			...
			TargetPort: port.TargetPort,  // exposed port of outside Pods
		})
	}
	...
	for _, svc := range currentBackendServices {
		...
		// We need to judge whether svc should be active arrording to the expected balancer object.
		// It yes, we just add it to activeBackendServices (which means it should keep running without change).
